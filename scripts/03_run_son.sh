#!/usr/bin/bash
# Chạy trên node son (replica 1).
# Lighthouse trên node vu phải đã chạy trước.
set -euxo pipefail

IMAGE="${IMAGE:-torchtitan:rocm7.2.4}"
VU_ADDR="${VU_ADDR:-10.130.0.13}"
SON_ADDR="${SON_ADDR:-10.130.0.2}"
LIGHTHOUSE_PORT="${LIGHTHOUSE_PORT:-29510}"
RDZV_PORT="${RDZV_PORT:-29501}"
TORCHFT_MANAGER_PORT="${TORCHFT_MANAGER_PORT:-29521}"
NGPU="${NGPU:-4}"
ROCR_VISIBLE_DEVICES="${ROCR_VISIBLE_DEVICES:-4,5,6,7}"
STEPS="${STEPS:-10000}"
RUN_NAME="${RUN_NAME:-qwen3_ft}"
CONTAINER_NAME="${CONTAINER_NAME:-torchtitan_ft_r1_son_${RUN_NAME}}"
SON_ROOT="${SON_ROOT:-/scratch/vunguyen13}"
DATA_ROOT="${DATA_ROOT:-${SON_ROOT}/tokenized_data}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${SON_ROOT}/codex_outputs}"
ASSET_ROOT="${ASSET_ROOT:-${SON_ROOT}/torchtitan/assets/hf}"

mkdir -p "${OUTPUT_ROOT}/${RUN_NAME}" "${ASSET_ROOT}"
...
docker run --rm \
    ...
    -v "${DATA_ROOT}:/workspace/torchtitan/tokenized_data:ro" \
    -v "${ASSET_ROOT}:/workspace/torchtitan/assets/hf" \
...
if [ ! -f ./assets/hf/Qwen3-0.6B/tokenizer.json ]; then
...
[job]
dump_folder = "./outputs/${RUN_NAME}/r1_son"
...
torchrun \
    ...
    --job.dump_folder "./outputs/${RUN_NAME}/r1_son" \
